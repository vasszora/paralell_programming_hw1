...
void MPISetup(unsigned *xmax, unsigned *ymax) {
  ...
  if (coords[0] == dims[0]-1) *xmax = static_cast<unsigned>((static_cast<int>(*xmax)) - (dims[0] - 1)*((static_cast<int>(*xmax))/dims[0]));
  else (*xmax) = static_cast<unsigned>((static_cast<int>(*xmax))/dims[0]);
  if (coords[1] == dims[0]-1) (*ymax) = static_cast<unsigned>((static_cast<int>(*ymax)) - (dims[1] - 1)*((static_cast<int>(*ymax))/dims[1]));
  else (*ymax) = static_cast<unsigned>((static_cast<int>(*ymax))/dims[1]);
